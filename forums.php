...
use local_edqscore\output\forums_page;
...
$pageurl = new moodle_url('/local/edqscore/forums.php', ['id' => $courseid]);
$PAGE->set_url($pageurl);
$PAGE->set_context($context);
$PAGE->set_pagelayout('report');
$PAGE->set_title(get_string('sectionforums', 'local_edqscore') . ': ' . format_string($course->fullname));
...
$PAGE->navbar->add(get_string('sectionforums', 'local_edqscore'));
...
$renderable = new forums_page($course, $scope, $cansettings, $pageurl, $forums);

echo $OUTPUT->header();
echo $OUTPUT->render_from_template('local_edqscore/forums_page', $renderable->export_for_template($OUTPUT));
echo $OUTPUT->footer();
