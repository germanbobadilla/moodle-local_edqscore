...
use local_edqscore\output\quizzes_page;
...
$pageurl = new moodle_url('/local/edqscore/quizzes.php', ['id' => $courseid]);
$PAGE->set_url($pageurl);
$PAGE->set_context($context);
$PAGE->set_pagelayout('report');
$PAGE->set_title(get_string('sectionquizzes', 'local_edqscore') . ': ' . format_string($course->fullname));
...
$PAGE->navbar->add(get_string('sectionquizzes', 'local_edqscore'));
...
$renderable = new quizzes_page($course, $scope, $cansettings, $pageurl, $quizzes);

echo $OUTPUT->header();
echo $OUTPUT->render_from_template('local_edqscore/quizzes_page', $renderable->export_for_template($OUTPUT));
echo $OUTPUT->footer();
